...
var TaskListTableDefault = function TaskListTableDefault(_ref) {
  ...
  return React__default.createElement("div", {
    ...
    if (isHorizontalDisplay) {
      if (t.start >= previousTaskEndDate) {
        newRow = false;
      }
...
var convertToBarTasks = function convertToBarTasks(tasks, dates, columnWidth, rowHeight, taskHeight, barCornerRadius, handleWidth, rtl, barProgressColor, barProgressSelectedColor, barBackgroundColor, barBackgroundSelectedColor, projectProgressColor, projectProgressSelectedColor, projectBackgroundColor, projectBackgroundSelectedColor, milestoneBackgroundColor, milestoneBackgroundSelectedColor, isHorizontalDisplay) {
  var barTasks = tasks.map(function (t, i) {
    ...
    return convertToBarTask(t, correctedIndex, dates, columnWidth, rowHeight, taskHeight, barCornerRadius, handleWidth, rtl, barProgressColor, barProgressSelectedColor, barBackgroundColor, barBackgroundSelectedColor, projectProgressColor, projectProgressSelectedColor, projectBackgroundColor, projectBackgroundSelectedColor, milestoneBackgroundColor, milestoneBackgroundSelectedColor);
  });
...
var TaskItem = function TaskItem(props) {
  var _props = _extends({}, props),
      ...
      isHorizontalDisplay = _props.isHorizontalDisplay,
      viewMode = _props.viewMode;

  var textRef = React.useRef(null);
  var displayNameShortenedRef = React.useRef(null);
  ...
  React.useEffect(function () {
    var isTextInside = true;

    if (task.x2 - task.x1 < 30) {
      isTextInside = false;
      displayNameShortenedRef.current = null;
    } else if (textRef.current) {
      var deltaWidth = textRef.current.getBBox().width - (task.x2 - task.x1);

      if (Math.floor(deltaWidth / 6) >= 1) {
        var charsToBeRemoved = Math.floor(deltaWidth / 6) + 3;

        var _displayNameShortened = task.name.substring(0, task.name.length - charsToBeRemoved);

        if (_displayNameShortened.length > 5) {
          displayNameShortenedRef.current = _displayNameShortened;
        } else {
          isTextInside = false;
          displayNameShortenedRef.current = null;
        }
...
var TaskGantt = function TaskGantt(_ref) {
  var gridProps = _ref.gridProps,
      ...
      scrollY = _ref.scrollY,
      scrollX = _ref.scrollX,
      barTaskIndexMax = _ref.barTaskIndexMax;
...
var Gantt = function Gantt(_ref) {
  ...
  var barTaskUniqueIndices = [];
  barTasks.forEach(function (barTask) {
    var alreadyInArray = false;
    barTaskUniqueIndices.forEach(function (barTaskUnique) {
